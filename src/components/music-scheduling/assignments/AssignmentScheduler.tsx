...
export const AssignmentScheduler = () => {
  ...
  const { data: assignmentsData, refetch: refetchAssignments } = useQuery(GET_MUSIC_CLOCK_ASSIGNMENTS, {
    variables: { networkId: currentNetwork?.id || '', filters: {} },
  });

  const [assignClock, { loading: assignLoading }] = useMutation(ASSIGN_MUSIC_CLOCK);
  ...
  const handleSlotToggle = useCallback((dayOfWeek: string, hour: number) => {
    ...
    setSelectedTimeSlots(prev => {
      const exists = prev.some(s => s.dayOfWeek === dayOfWeek && s.hour === hour);
      return exists
  ...
  const isSlotSelected = useCallback((dayOfWeek: string, hour: number) => {
    return selectedTimeSlots.some(s => s.dayOfWeek === dayOfWeek && s.hour === hour);
  }, [selectedTimeSlots]);

  const getAssignedClock = useCallback((dayOfWeek: string, hour: number) => {
  ...
  const handleAssignment = useCallback(() => {
    ...
    Promise.all(assignmentPromises)
      .then((results) => {
        const successful = results.filter(result => result.data?.assignMusicClock?.assignment);
        if (successful.length === selectedTimeSlots.length) {
          toast('Clock assignments created successfully', 'success');
        } else {
          toast(`Created ${successful.length} of ${selectedTimeSlots.length} assignments`, 'warning');
        }
        setSelectedClock('');
  ...
  }, [selectedClock, selectedTimeSlots, currentNetwork?.id, assignClock, refetchAssignments]);
      ...
      <Card>
        <div className="assignment-scheduler__header">
          <div className="assignment-scheduler__title">
            <h2 className="assignment-scheduler__heading">Clock Assignment Scheduler</h2>
            <p className="assignment-scheduler__description">Assign music clocks to specific time slots</p>
          </div>
        </div>

        <div className="assignment-scheduler__controls">
            <div className="form-field">
              <label htmlFor="clock-select" className="form-label">Select Clock</label>
              <select
                id="clock-select"
                value={selectedClock}
              ...
              >
                ...
                {clocksData?.musicClocks?.map(clock => (
                  <option key={clock.id} value={clock.id}>
                    {clock.name} ({Math.floor(clock.duration / 60)}min)
                  </option>
                ))}
              </select>
            </div>
            <div className="assignment-scheduler__actions">
              <Button
                ...
                disabled={!selectedClock || selectedTimeSlots.length === 0 || assignLoading}
        ...
        <div className="assignment-scheduler__legend">
          ...
          <div className="assignment-total">
            Total assignments: {assignmentsData?.musicClockAssignments?.length || 0}
          </div>
        </div>
      </Card>
    </div>
