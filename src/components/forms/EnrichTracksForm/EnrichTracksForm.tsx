...
export function EnrichTracksForm({
  ...
  const [currentTrackForSearch, setCurrentTrackForSearch] =
    useState<Track | null>(null);
  const [musicBrainzModal, setMusicBrainzModal] = useState(false);
  const [pendingEnrichments, setPendingEnrichments] = useState<
    PendingEnrichment[]
  >([]);
  ...
  const filteredSubcategoryOptions = useMemo(() => {
    ...
    return subcategoryOptions.filter((option) => {
      if (option.value === '') return true; // Always show "All Categories"
      return (
  ...
  const handleMusicBrainzSelect = useCallback(
    async (result: MusicBrainzSearchResult) => {
      ...
      if (activeTab === 'single') {
        ...
        try {
          ...
          onSuccess?.(
            `Created enrichment job for "${currentTrackForSearch.artist} - ${currentTrackForSearch.title}"`,
          );
        ...
        if (nextIndex < selectedTrackIds.length) {
          const nextTrack = tracks.find(
            (t) => t.id === selectedTrackIds[nextIndex],
          );
          if (nextTrack) {
  ...
  const handleExecutePendingEnrichments = useCallback(async () => {
    ...
    try {
      ...
      onSuccess?.(
        `Created ${pendingEnrichments.length} enrichment jobs for bulk processing`,
      );
      ...
      <div className="enrich-tracks-form__container">
            ...
            {trackId ? (
              <>
                ...
                <p className="enrich-tracks-form__instructions">
                  You were directed to enrich a specific track. The MusicBrainz
                  search should open automatically.
                </p>
              </>
            ) : (
              <>
                ...
                <p className="enrich-tracks-form__instructions">
                  Click the &ldquo;Enrich&rdquo; button on any track to search
                  for its metadata in MusicBrainz.
                </p>
              </>
            )}
        ...
        <Tabs.Content value="bulk">
          <div className="enrich-tracks-form__bulk">
            ...
            <p className="enrich-tracks-form__instructions">
              Select multiple tracks, search MusicBrainz for each one, then
              execute all enrichment jobs at once.
            </p>
            ...
            {pendingEnrichments.length > 0 && (
              <div className="enrich-tracks-form__pending">
                ...
                <div className="enrich-tracks-form__pending-list">
                  {pendingEnrichments.map((pending) => (
                    <div
                      ...
                      className="enrich-tracks-form__pending-item"
                    >
                      <div className="enrich-tracks-form__pending-track">
                        <strong>
                          {pending.track.artist} - {pending.track.title}
                        </strong>
                      </div>
                      <div className="enrich-tracks-form__pending-release">
                        → {pending.musicbrainzResult.artist} -{' '}
                        {pending.musicbrainzResult.title} (
                        {pending.musicbrainzResult.score}%)
                      </div>
                      <Button
                        size="sm"
                        variant="secondary"
                ...
                <div className="enrich-tracks-form__pending-actions">
                  ...
                  <Button variant="secondary" onClick={handleClearPending}>
                    Clear All Pending
                  </Button>
                </div>
              </div>
